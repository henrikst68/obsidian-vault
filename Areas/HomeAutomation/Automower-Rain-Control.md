...
2. **entities** "Nowcast status" — condition, `has_precipitation`, `radar_online`, `radar_coverage` attributes of `weather.met_no_nowcast_sandgraven`. **(removed 2026-06-13 — chart kept, status card deleted per request.)**
...
- **UPDATE (2026-06-14): "configuration error" on Android — fixed.** v3.6.5 removed the legacy single-marker keys (`marker_latitude`, `marker_longitude`, `show_marker`) in favor of a `markers[]` array, and also doesn't use `card_title`/`frame_count` (0 refs in bundle). Old config rejected by the current bundle (Android loaded fresh JS; desktop had cached older tolerant build). Rewrote config to v3 schema: `markers: [{latitude, longitude, color, label}]` (default home icon auto dark/light), removed stale keys. No title key exists in this version. Android needs app cache clear / app restart to drop the old bundle. Valid bundle keys confirmed: center_latitude/longitude, zoom_level, frame_delay, restart_delay, show_range/zoom/recenter/playback/scale, map_style, data_source, markers.
